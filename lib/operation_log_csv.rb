...
require 'logger'
  ...
  def self.class_for(type,organization = nil)
    detect_class(type, organization) ||
      ...
      "OperationLogCsv::#{type}".constantize
  end

  def self.detect_class(type, organization)
    "OperationLogCsv::#{name_for(type,organization)}".constantize if organization rescue nil
  end
 ...
 def self.log
    Logger.new('output_logs/OpLogGeneration.log', 'daily')
  end
end
