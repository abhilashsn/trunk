...
require 'logger'
  ...
  def self.class_for(type,organization = nil)
    detect_class(type, organization) ||
      ...
      "OutputPcPrint::#{type}".constantize
  end

  def self.detect_class(type, organization)    
    "OutputPcPrint::#{name_for(type,organization)}".constantize if organization rescue nil
  end
  #Takes in the facility/client/partner name and the type of PcPrint subclass
  ...
  def self.name_for(type,organization)
   ...
   def self.log
    Logger.new('output_logs/PcPrintGeneration.log', 'daily')
  end
end