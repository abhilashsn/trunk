...
module OutputXml::MedistreamsTemplateVariableTranslations
  ...
  def group_id
    @batch.details[:group_id].rr_blank_or_null? ? @facility.sitecode : @batch.details[:group_id]
  end
  ...
  def interchange_receiver_id
    if  @batch.details[:interchange_receiver_id].rr_blank_or_null?
      if @facility.name.upcase == "ASSOCIATED EMERGENCY PHYSICIANS"
        receiver_id = "AEP"
      elsif @facility.name.upcase == "PUGET SOUND PHYSICIANS"
        receiver_id = "PSP"
      else
        receiver_id = @facility.name[0..14]
      end
    else
      receiver_id =  @batch.details[:interchange_receiver_id]
    end
    return receiver_id
  end
  ...
  def group_receiver_id
    if  @batch.details[:group_receiver_id].rr_blank_or_null?
      if @facility.name.upcase == "ASSOCIATED EMERGENCY PHYSICIANS" || @facility.name.upcase == "PUGET SOUND PHYSICIANS"
        group_receiver_id = "EMEDEX"
      else
        group_receiver_id = @facility.name[0..14]
      end
    else
      group_receiver_id = @batch.details[:group_receiver_id]
    end
