# Script to set end time of file posting to FTP in output_activity_logs given file name and time.
...
optparse = OptionParser.new do|opts|
  opts.banner = "Usage: ruby script/file_posting_end_time.rb [options], File Name and Time are mandatory"

  opts.on( '-f', '--file FILE NAME', 'File Name' ) do |file|
    @options[:file_name] = file
  end

  opts.on( '-t', '--time TIME', "File Posting End Time 'YYYY-MM-DD HH:MM:SS'" ) do |time|
...
if @options[:file_name].nil? || @options[:end_time].nil?  
  puts error_message
  @log.error error_message
else
  begin
    @log.debug "#{@options[:file_name]} : End Time = '#{@options[:end_time]}'"
    converted_time = Time.strptime("#{@options[:end_time]}", '%Y-%m-%d %H:%M:%S').utc
    time_array = converted_time.to_s.split(' ')
    time_array.delete_at(-1)
    converted_time = time_array.join(" ")
    query = "UPDATE output_activity_logs SET upload_end_time = '#{converted_time}' WHERE file_name = '#{@options[:file_name]}'"
